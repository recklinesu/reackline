...
const FetchEvents = async () => {
    const api = "https://api.recklinesports.com/api/1-5-B-S-P-C-F-S-0-0-5/fetch-events";
...
const FetchLegues = async (events) => {
    ...
    await Promise.all(events.map(async (event) => {
        try {
            const api = "https://api.recklinesports.com/api/1-5-B-S-P-C-F-S-0-0-5/fetch-compatitions/" + parseInt(event.eventType);
            const response = await axios.get(api);
...
const FetchMatches = async (leagues) => {
    ...
    await Promise.all(leagues.map(async (event) => {
        try {
            const api = "https://api.recklinesports.com/api/1-5-B-S-P-C-F-S-0-0-5/fetch-matches/" + parseInt(event.eventType) + "/" + parseInt(event.competition.id);
            const response = await axios.get(api);
